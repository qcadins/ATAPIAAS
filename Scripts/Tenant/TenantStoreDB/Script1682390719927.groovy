...
ArrayList<String> result = CustomKeywords.'tenant.tenantVerif.getTenantStoreDB'(connProd, findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 14))

'get data services dari DB'
ArrayList<String> resultServices = CustomKeywords.'tenant.tenantVerif.getTenantServicesDescription'(connProd, findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 13))

'declare arrayindex'
arrayindex = 0

'verify tenant name'
arrayMatch.add(WebUI.verifyMatch(findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 12).toUpperCase(), (result[arrayindex++]).toUpperCase(),
		false, FailureHandling.CONTINUE_ON_FAILURE))

if(findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 7).equalsIgnoreCase('New')){
	
	'verify tenant code'
	arrayMatch.add(WebUI.verifyMatch(findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 13).toUpperCase(), (result[arrayindex++]).toUpperCase(),
			false, FailureHandling.CONTINUE_ON_FAILURE))
}else {
	'skip'
	arrayindex++
...
arrayMatch.add(WebUI.verifyMatch(findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 14).toUpperCase(), (result[arrayindex++]).toUpperCase(),
		false, FailureHandling.CONTINUE_ON_FAILURE))

'verify API Key'
arrayMatch.add(WebUI.verifyMatch(findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 16).toUpperCase(), (result[arrayindex++]).toUpperCase(),
		false, FailureHandling.CONTINUE_ON_FAILURE))

'verify Email reminder'
arrayMatch.add(WebUI.verifyMatch(findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 21).toUpperCase().replace(';',','), (result[arrayindex++]).toUpperCase(),
		false, FailureHandling.CONTINUE_ON_FAILURE))

ArrayList<String> arrayServices = findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 18).split(';',-1)

ArrayList<String> arrayServicesBatasSaldo = findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 19).split(';',-1)
...
for(indexExcel = 0 ; indexExcel < arrayServices.size(); indexExcel++) {
	...
	String result = CustomKeywords.'tenant.tenantVerif.getTenantServices'(connProd, findTestData(ExcelPathTenant).getValue(GlobalVariable.NumOfColumn, 9)).replace('{','').replace('}','').replace('"','').replace(',','')
