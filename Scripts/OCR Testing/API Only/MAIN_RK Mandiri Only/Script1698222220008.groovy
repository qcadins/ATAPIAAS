...
'get base url'
GlobalVariable.BaseUrl =  findTestData('Login/BaseUrl').getValue(2, rowExcel('OCR RK Mandiri'))

'mencari directory excel utama'
GlobalVariable.DataFilePath = CustomKeywords.'writetoexcel.WriteExcel.getExcelPath'('/Excel/2. APIAAS.xlsx')

sheet = 'OCR RK Mandiri'
...
for (GlobalVariable.NumOfColumn = 2; GlobalVariable.NumOfColumn <= countColumnEdit; (GlobalVariable.NumOfColumn)++) {
	...
	if (findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, rowExcel('Status')).length() == 0) {
		...
		response = WS.sendRequest(findTestObject('Object Repository/OCR Testing/New API/Statement Mandiri',
		[
		...
		if (!state.equalsIgnoreCase('key or tenant invalid') &&
			...
			'jika tanpa key dan tenant yang benar sukses hit, tulis error bypass'
