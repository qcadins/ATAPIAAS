...
'get base url'
GlobalVariable.BaseUrl =  findTestData('Login/BaseUrl').getValue(2, rowExcel('OCR STNK'))
...
for (GlobalVariable.NumOfColumn = 2; GlobalVariable.NumOfColumn <= countColumnEdit; (GlobalVariable.NumOfColumn)++) {
	...
	if (findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, rowExcel('Status')).length() == 0) {
		...
		String thekey = CustomKeywords.'ocrtesting.GetParameterfromDB.getAPIKeyfromDB'(conn,
			tenantcode, GlobalVariable.SettingEnvi)
		
		'deklarasi variable response'
		ResponseObject response
		
		'cek apakah perlu tambah API'
		String useCorrectKey = findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn,
			rowExcel('UseCorrectKey?(Yes/No)'))
		
		'cek apakah perlu gunakan tenantcode yang salah'
		String useCorrectTenant = findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn,
			rowExcel('UseCorrectTenantCode?'))
		...
		response = WS.sendRequest(findTestObject('Object Repository/OCR Testing/New API/STNK',
		[
		...
		CustomKeywords.'writetoexcel.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet,
			rowExcel('Process Time') - 1, GlobalVariable.NumOfColumn - 1, elapsedTime.toString())
		...
		if ((!stateocr.equalsIgnoreCase('key or tenant invalid') && !messageocr.equalsIgnoreCase('Invalid API key or tenant code')) &&
			...
			if (GlobalVariable.checkLog == 'Yes') {
				...
				WebUI.callTestCase(findTestCase('OCR Testing/API Only/CheckLog'), [('OCRType') : 'OCR_stnk',
