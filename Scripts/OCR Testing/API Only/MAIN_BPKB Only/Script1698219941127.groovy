...
'mendapat jumlah kolom dari sheet BPKB'
...
String message_ocr, numofpages_ocr, state_ocr, date_ocr, read_ocr, readconfidence_ocr
...
for (GlobalVariable.NumOfColumn = 2; GlobalVariable.NumOfColumn <= countColumnEdit; (GlobalVariable.NumOfColumn)++) {
	...
	if (findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, rowExcel('Status')).length() == 0) {
		...
		numofpages_ocr = WS.getElementPropertyValue(response, 'num_of_pages')
