...
WebUI.callTestCase(findTestCase('Test Cases/Login/Login'), [('TC') : 'OCR', ('SheetName') : 'OCR Face Compare',
	('Path') : ExcelPathOCRTesting, ('Row') : 27], FailureHandling.STOP_ON_FAILURE)
...
for(GlobalVariable.NumOfColumn = 2; GlobalVariable.NumOfColumn <= countColumnEdit; (GlobalVariable.NumOfColumn)++){
	...
	if (findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 1).length() == 0) {
		...
		int idPayment = CustomKeywords.'ocrTesting.GetParameterfromDB.getIDPaymentType'(conndevUAT,
			tenantcode, findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 12))
		...
		String useCorrectKey = findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 17)
		
		'cek apakah perlu gunakan tenantcode yang salah'
		String useCorrectTenant = findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 15)
		...
		saldobefore = getSaldoforTransaction(findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 12))
		
		if (useCorrectKey != 'Yes') {
			
			thekey = findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 18)
		}
		if (useCorrectTenant != 'Yes') {
			
			tenantcode = findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 16)
		}
		...
		[	('img1'): findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 9),
			('img2'): findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 10),
			('key'):thekey,
			('tenant'):tenantcode,
			('refNum'):findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 11),
			('source'):findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 20),
			('loginid'):findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 21),
			('nik'):findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 22),
			('offcode'):findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 23),
			('offname'):findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 24),
			('question'):findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 25),
		]))
			
		'ambil message respon dari HIT tersebut'
		String message_ocr = WS.getElementPropertyValue(response, 'error')
		...
		String verifState_ocr = WS.getElementPropertyValue(response, 'message')
	
		'jika kurang saldo hentikan proses testing'
		if (state_ocr.equalsIgnoreCase('Failed') && verifState_ocr.equalsIgnoreCase('Insufficient balance')) {
			
			'write to excel status failed dan reason'
			CustomKeywords.'writeToExcel.WriteExcel.writeToExcelStatusReason'('OCR Face Compare', GlobalVariable.NumOfColumn,
			...
			if(GlobalVariable.SettingTopup.equals('IsiSaldo')) {
				...
				'call auto isi saldo'
				WebUI.callTestCase(findTestCase('IsiSaldo/IsiSaldoAuto'), [('ExcelPathOCR') : ExcelPathOCRTesting, ('ExcelPath') : 'Login/Login', ('tipeSaldo') : 'Face Compare', ('sheet') : 'OCR Face Compare', ('idOCR') : 'FACE_COMPARE'],
					FailureHandling.CONTINUE_ON_FAILURE)
			}
			else if (GlobalVariable.SettingTopup.equals('SelfTopUp')) {
				
				'call isi saldo secara mandiri di Admin Client'
				WebUI.callTestCase(findTestCase('Top Up/TopUpAuto'), [('ExcelPathOCR') : ExcelPathOCRTesting, ('ExcelPath') : 'Login/Login', ('tipeSaldo') : 'Face Compare', ('sheet') : 'OCR Face Compare', ('idOCR') : 'FACE_COMPARE'],
					FailureHandling.CONTINUE_ON_FAILURE)
				
				'lakukan approval di transaction history'
				WebUI.callTestCase(findTestCase('Transaction History/TransactionHistoryAuto'), [('ExcelPathOCR') : ExcelPathOCRTesting, ('ExcelPath') : 'Login/Login', ('tipeSaldo') : 'Face Compare', ('sheet') : 'OCR Face Compare', ('idOCR') : 'FACE_COMPARE'],
					FailureHandling.CONTINUE_ON_FAILURE)
			}
		...
		else if (state_ocr.equalsIgnoreCase('Success') && useCorrectKey != 'Yes' && useCorrectTenant != 'Yes') {
		...
		'simpan harga Face Compare ke dalam integer'
		...
		uiSaldoafter = getSaldoforTransaction(findTestData(ExcelPathOCRTesting).getValue(GlobalVariable.NumOfColumn, 12))
		...
		if (message_ocr.equalsIgnoreCase('') && state_ocr.equalsIgnoreCase('Success') && isTrxIncreased == 1 && isSaldoBerkurang == 1 && HitAPITrx == 1) {
			
			'tulis status sukses pada excel'
			CustomKeywords.'writeToExcel.WriteExcel.writeToExcelStatusReason'('OCR Face Compare',
		...
		else if (state_ocr.equalsIgnoreCase('Success') && isTrxIncreased == 0 && isSaldoBerkurang == 1) {
		...
		else if (state_ocr.equalsIgnoreCase('Success') && isTrxIncreased == 1 && isSaldoBerkurang == 0) {
		...
		else if (HitAPITrx == 0 && state_ocr.equalsIgnoreCase('Success') && isTrxIncreased == 0
			&& isSaldoBerkurang == 1) {
